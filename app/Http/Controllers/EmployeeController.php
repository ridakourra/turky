...
class EmployeeController extends Controller
{
    ...
    public function show(Request $request, Employee $employee)
    {
        ...
        // Get historique travail with pagination
        $historiqueQuery = HistoriqueTravail::whereHas('salaire', function ($query) use ($employee) {
            $query->where('employee_id', $employee->id);
        })->with(['salaire.produit'])->orderBy('date', 'desc');

        $historiqueTravail = $historiqueQuery->paginate(10, ['*'], 'historique_page');
        ...
        return Inertia::render('Employees/Show', [
            ...
            'historiqueTravail' => $historiqueTravail,
            'produits' => Produit::all()
        ]);
    }
    ...
    public function storeBudget(Request $request, Employee $employee)
    {
        ...
        // Check if budget already exists, if so update it, otherwise create new one
        if ($employee->budgetChauffeur) {
            $employee->budgetChauffeur->update($validated);
        } else {
            $employee->budgetChauffeur()->create($validated);
        }

        return back()->with('success', 'Budget ajouté avec succès.');
    }

    public function updateBudget(Request $request, Employee $employee, BudgetChauffeur $budget)
    {
        $validated = $request->validate([
            'montant' => 'required|numeric|min:0',
            'description' => 'nullable|string',
            'date_attribution' => 'required|date',
            'statut' => 'required|in:attribue,utilise,solde'
        ]);

        $budget->update($validated);

        return back()->with('success', 'Budget mis à jour avec succès.');
    }
    ...
    public function storeHistoriqueTravail(Request $request, Employee $employee)
    {
        ...
        // Verify that the salaire belongs to this employee
        $salaire = Salaire::where('id', $validated['salaire_id'])
                         ->where('employee_id', $employee->id)
                         ->first();

        if (!$salaire) {
            return back()->withErrors(['salaire_id' => 'Le salaire sélectionné n\'appartient pas à cet employé.']);
        }
