...
export default function Edit({ vehicule, chauffeurs }) {
  // نملأ النموذج بقيم الـ vehicule الحالية
  const { data, setData, patch, processing, errors, reset } = useForm({
    nom: vehicule.nom || "",
    photo: null, // نتركها فارغة حتى لا تُغيّر إلا بعد رفع جديد
    matricule: vehicule.matricule || "",
    type: vehicule.type || "",
    chauffeur_id: vehicule.chauffeur_id ? String(vehicule.chauffeur_id) : "none",
    capacite_tonne: vehicule.capacite_tonne || "",
    consommation_litre_par_km: vehicule.consommation_litre_par_km || "",
    actif: vehicule.actif,
    notes: vehicule.notes || "",
  });

  // معاينة الصورة الحالية والمُعدّلة
  const [previewUrl, setPreviewUrl] = useState(`/storage/${vehicule.photo}` || null);
  ...
  const handlePhotoChange = (e) => {
    ...
    if (file) {
      ...
      // إذا ألغى المستخدم رفع الصورة، نعيد المعاينة للصورة القديمة
      setPreviewUrl(vehicule.photo || null);
    }
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    // نحول "none" إلى سلسلة فارغة
    const payload = {
      ...data,
      chauffeur_id: data.chauffeur_id === "none" ? "" : data.chauffeur_id,
    };
    router.post(route("vehicules.update", vehicule.id), {
      _method: "PUT", // الطريقة المطلوبة في Laravel
      ...data,
      chauffeur_id: data.chauffeur_id === "none" ? "" : data.chauffeur_id,
    }, {
      forceFormData: true,
      onStart: () => console.log("Envoi..."),
    });
  };

  // إذا لم يرفع المستخدم صورة جديدة وأراد إعادة الضبط، نعيد المعاينة للصورة الأصلية
  const handleReset = () => {
    reset();
    setPreviewUrl(vehicule.photo || null);
  };
  ...
  return (
    <AdminLayout titleHead={`Modifier ${vehicule.nom}`} title="Modifier un Véhicule">
      <Head title="Edit Vehicule" />
